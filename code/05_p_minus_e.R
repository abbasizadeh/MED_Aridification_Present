...
path_save <- "~/shared/data_projects/med_datasets/2000_2019_data/p_minus_e/"


precip_obs_files <- list.files(path_obs_precip)
precip_sim_files <- list.files(path_sim_precip)
evap_sim_files <- list.files(path_sim_evap)
...
for(name in 1:length(precip_sim_files)){
  ...
  p_data_frame$raster[name] <- list(brick(paste0(path_sim_precip, precip_sim_files[name])))
}
...
for(name in  itr_start:itr_end){
  ...
  p_data_frame$raster[name] <- list(brick(paste0(path_obs_precip, precip_obs_files[[index]])))
}
...
for(name in 1:length(evap_sim_files)){
  ...
  if(!startsWith(paste0(evap_names_table$V2[name], "_", evap_names_table$V1[name]), "pet")){
    ...
    e_data_frame$raster[name] <- list(brick(paste0(path_sim_evap, evap_sim_files[name])))
  }
...
for(p_itr in 1:length(p_data_frame$p_raster_name)) {
  for (e_itr in 1:length(e_data_frame$e_raster_name)) {
    ...
    save_nc(p_minus_e_dummie, paste0(path_save, new_name, ".nc"))
